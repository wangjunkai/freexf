...
angular.module('freexf')
	.controller('lottery_ctrl', function($scope, $rootScope, $injector, $location, $timeout, $state, $ionicPopup, $Loading, $stateParams, $ionicScrollDelegate, AUTH, ENV, MSGICON) {
		...
		var iosFun = function(iosdata) {
			params.StudentId=iosdata,
			getDiscounts(params);
			window.mbStyle = 'ios';
		}
		if(navigator.userAgent.match(/(Android)/i)) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				<!--注册，获取Android来的数据-->
				bridge.registerHandler("AndroidToHtml", function(data, responseCallback) {
					params.StudentId= data;
					getDiscounts(params);
					responseCallback(data);
					responseCallback('Html获取成功');
				});
			});
			...
			function sendDataToAndroid(data) {
				window.WebViewJavascriptBridge.callHandler(
					...
					function(responseData) {
						document.getElementById("showText").innerHTML = '反馈数据：' + responseData;
					}
				);
			}
			window.mbStyle = 'and';
		...
		$scope.goPay = function() {
			...
			if(window.mbStyle == 'ios') {
				window.webkit.messageHandlers.lottery.postMessage($scope.resultData.DiscountCode)
			} else {
			    sendDataToAndroid($scope.resultData.DiscountCode);
			}
