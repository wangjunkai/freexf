/// <reference path="classLearning.html" />
'use strict';

angular.module('freexf')
  .controller('classlearning_ctrl', function ($scope, $rootScope, $state, AUTH, $freexfUser, ENV, DispatchRepository, $frModal) {
      ...
      $rootScope.$on('auth:update', function (event, auth) {
          $scope.userData = auth;
          var element = '还有' + extractingTimes + '次机会';
          $scope.msg = element;
      });
      //计算转盘指针位置
      ...
      var rotate = $('#rotate')
      var bg = $('#turntable-bg');
      console.log(rotate.width() + '*' + rotate.height())
      var top = (rotate.width() - pointer.width() * 1.3) / 2 + 1 * winHeight / 70;
      var left = (rotate.width() - pointer.width()) / 2 + 20;
      $('#pointer').css({ 'top': top + 'px', 'left': left + 'px' });
      b.css('margin-top', -(pointer.width() * 1.3 * 0.55) + 'px');
      p.css('margin-top', (15 * winHeight / 800) + 'px');
      //rotate.css('padding-top', (bg.width() - rotate.width()) / 2);

    //规则、记录位置
    $('.rulecontent ion-scroll').css('height', bg.width() * 0.2);
    $('#contentbox').css('height', bg.width() * 0.15);
    $('.listcontent').css('height', bg.width() * 0.15 + 66);
    ...
    //抽奖转盘
    var StudentId = $scope.userData.rowId;
    ...
    function myWinning(data) {
      ...
      if (data.length == 0) {
        ...
        $(data).each(function () {
          var name = this.NickName.substring(0, 3) + '****' + this.NickName.substring(7, 11);
          var liele = $('<li><span class="prize"></span><span class="time"></span></li>').appendTo($('.mylistcontent ion-scroll .mylistcontent-lisibox'));
