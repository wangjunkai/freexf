//日历生成控件
var CalendarFun = function(thisy, thism, thisd,obj,today) {
	var thetr='';
	for (var i = 0; i < 6; i++) {
		thetr=thetr+'<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>'
	};
	if (typeof ($(obj).attr('firstDay')) != 'undefined') {
	    var firstDay = $(obj).attr('firstDay');
	} else {
	    var firstDay = 0;
	}
	
	$(obj).html(thetr);
	$(obj).find('tr').addClass('calendar-date');
	$(obj).find('td').addClass('date-item');
	$(obj).find('td').each(function(i) {
		$(this).html('<font></font><font></font>')
		$(this).attr('id', 'GD' + i)
		$(this).find('font').eq(0).attr('id', 'SD' + i)
		$(this).find('font').eq(1).attr('id', 'LD' + i)
	})
	...
	function calendar(y, m) {
		...
		this.firstWeek = sDObj.getDay() - firstDay; //公历当月1日星期几
	...
	function drawCld(SY, SM) {
		...
		for (i = 0; i < 42; i++) {
			...
			$(ggObj).attr('YMDday', SYst + '' + ggSMst + '' + ggsD)
			if (cld[sD].isToday) {
			...
			for (var ipp = 0; ipp < sFtv.length; ipp++) { //公历节日
				if (parseInt(sFtv[ipp].substr(0, 2)) == (SMst)) {
	...
	//用自定义变量保存当前系统中的年月日
	var Today = new Date(thisy, thism - 1, thisd);
