...
angular.module('freexf', ['ionic'])
  ...
  .controller('coursedetail_ctrl', function ($scope, $sce, $rootScope, $injector, $ionicLoading, $ionicPopup, $ionicScrollDelegate, $timeout, $state, $stateParams, AUTH, ENV, CourseDateRepository, AddMyFavoriteRepository, DelMyFavoriteRepository, AddFlower, RemoveFlower) {
    ...
    CourseDate.getModel(params).then(function (res) {
      ...
      $scope.teacherPath = !$scope.courseDate.teacherPath[0] ? 'img/course/teacher_img.png' : $scope.courseDate.teacherPath[0];
      $scope.teacher = $scope.courseDate.teacher.length == 0 ? false : true
      var arr = [];
      //$scope.courseDate.IsBoughtCrouse = true;    //已购买
      if ($scope.courseDate.isPermissionCrouse == true) {   //isPermissionCrouse开发者用户
        $scope.buy = true;
        $scope.isbuy = true;
